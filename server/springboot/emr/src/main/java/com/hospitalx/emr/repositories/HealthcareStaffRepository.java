...
public interface HealthcareStaffRepository extends MongoRepository<HealthcareStaff, String> {
        ...
        @Query(value = "{ 'staffType' : ?0, 'fullName' : { $regex: ?1, $options: 'i' }, 'title' : ?2, 'departmentId' : ?3, 'gender': ?4, 'deleted' : false}")
        public Page<HealthcareStaff> findByDoctor(StaffType staffType, String fullName, String title,
                        String departmentId,
                        String gender, Pageable pageable);

        @Query(value = "{ 'staffType' : ?0, 'fullName' : { $regex: ?1, $options: 'i' }, 'title' : ?2, 'departmentId' : ?3, 'gender': ?4, 'schedules': { $exists: true }, 'deleted' : false}")
