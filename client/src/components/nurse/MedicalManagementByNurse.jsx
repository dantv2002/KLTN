...
import { createMedical, getMedicalNurDoc, getRecordNurDoc, getDepartmentNurDoc, getMark, updateMedical } from "../../Api";
import { SearchOutlined, PlusCircleOutlined } from '@ant-design/icons';

const { Panel } = Collapse;

const MedicalManagementByNurse = () => {
  ...
  const [visibleReadRecord, setVisibleReadRecord] = useState(false);
  const [idRecordOutpatient, setIdRecordOutpatient] = useState("");
  const [visibleInsertOutpatient, setVisibleInsertOutpatient] = useState(false);
  const [formInsertOutpatient] = Form.useForm();
  const [idRecordInpatient, setIdRecordInpatient] = useState("");
  const [visibleInsertInpatient, setVisibleInsertInpatient] = useState(false);
  const [formInsertInpatient] = Form.useForm();
  ...
  const columnsRecords = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button type="link" className="outpatient" onClick={() => handleReadOutpatient(record.Id)}>
            Tạo hs ngoại trú
          </Button>
          <Button type="link" className="inpatient" onClick={() => handleReadInpatient(record.Id)}>
            Tạo hs nội trú
          </Button>
        </Space>
      ),
  ...
  const handleCancelReadRecord = () => {
    setVisibleReadRecord(false);
    setKeywordRecord("");
    setGenderRecord("");
    setYearRecord("");
  ...
  const handleReadOutpatient = (id) => {
    setIdRecordOutpatient(id)
    setVisibleInsertOutpatient(true);
  }

  const handleCancelInsertOutpatient = () => {
    setIdRecordOutpatient("");
    formInsertOutpatient.resetFields();
    setVisibleInsertOutpatient(false);
  }

  const handleCreateOutpatient = async(values) => {
    const { Reason, PathologicalProcess, MedicalHistory, HeartRate, Temperature, BloodPressure, RespiratoryRate, Weight, Date, InitialDiagnosis } = values;
    const data = {
      Reason,
      PathologicalProcess,
      MedicalHistory,
      ...
      InitialDiagnosis,
      RecordId: idRecordOutpatient,
    };
    
    try {
      const response = await axios.post(createMedical, data, { withCredentials: true });
      if (response.status === 200) {
        message.success(response.data.Message);
        setVisibleInsertOutpatient(false);
        formInsertOutpatient.resetFields();
  ...
  const handleReadInpatient = (id) => {
    setIdRecordInpatient(id)
    setVisibleInsertInpatient(true);
  }

  const handleCancelInsertInpatient = () => {
    setIdRecordInpatient("");
    formInsertInpatient.resetFields();
    setVisibleInsertInpatient(false);
  }

  const handleCreateInpatient = async (values) => {
    const { 
      Reason, 
      PathologicalProcess, 
      MedicalHistory, 
  ...
  return (
    <div>
      ...
      <br/>
      <Button onClick={() => handleReadRecord()} className="bg-cyan-400 text-black mt-4 mb-4" htmlType="submit" icon={<PlusCircleOutlined />} >Tạo bệnh án</Button>
      <Table 
      ...
      <Modal 
        title={<h1 className="text-2xl font-bold text-blue-700 text-center mb-4">Tạo hồ sơ ngoại trú</h1>}
        visible={visibleInsertOutpatient}
        onOk={() => formInsertOutpatient.submit()}
        okText="Tạo"
        onCancel={handleCancelInsertOutpatient}
        cancelText="Thoát"
        ...
        <Form {...formLayout} form={formInsertOutpatient} onFinish={handleCreateOutpatient} className="space-y-4">
          <Collapse className="w-full">
            <Panel header="Thông tin chung" key="1">
              ...
              <Form.Item name="PathologicalProcess" label="Quá trình bệnh lý" rules={[{ required: true, message: 'Vui lòng nhập quá trình bệnh lý' }]} className="w-full">
                <Input />
              </Form.Item>
      ...
      <Modal 
        title={<h1 className="text-2xl font-bold text-blue-700 text-center mb-4">Tạo hồ sơ nội trú</h1>}
        visible={visibleInsertInpatient}
        onOk={() => formInsertInpatient.submit()}
        okText="Tạo"
        onCancel={handleCancelInsertInpatient}
        cancelText="Thoát"
        okButtonProps={{ className: "bg-blue-700" }}
        cancelButtonProps={{ className: "bg-red-600" }}
        width={650}
        ...
        <Form {...formLayout} form={formInsertInpatient} onFinish={handleCreateInpatient} className="space-y-4">    
          <Collapse className="w-full">
            <Panel header="Thông tin chung" key="1">
              ...
              <Form.Item name="Prognosis" label="Tiên lượng" rules={[{ required: true, message: 'Vui lòng nhập dự đoán' }]} className="w-full">
                <Input />
              </Form.Item>
            </Panel>
          </Collapse>
          <Collapse className="w-full">
            <Panel header="Thông tin nhập viện" key="2">
              <Form.Item name="DateAdmission" label="Thời gian" rules={[{ required: true, message: 'Vui lòng chọn thời gian nhập viện' }]} className="w-full">
                <DatePicker className="w-full" showTime format="HH:mm DD/MM/YYYY" />
              </Form.Item>
              <Form.Item name="DepartmentAdmission" label="Khoa" rules={[{ required: true, message: 'Vui lòng nhập phòng nhập viện' }]} className="w-full">
              ...
              <Form.Item name="DiagnosisAdmission" label="Chẩn đoán" rules={[{ required: true, message: 'Vui lòng nhập chẩn đoán nhập viện' }]} className="w-full">
                <Input />
              </Form.Item>
            </Panel>
          </Collapse>
          <Collapse className="w-full">
            <Panel header="Thông tin chuyển khoa" key="3">
              <Form.Item name="DateTransfer" label="Thời gian" rules={[{ required: true, message: 'Vui lòng chọn thời gian chuyển viện' }]} className="w-full">
                <DatePicker className="w-full" showTime format="HH:mm DD/MM/YYYY" />
              </Form.Item>
              <Form.Item name="DepartmentTransfer" label="Khoa" rules={[{ required: true, message: 'Vui lòng nhập phòng chuyển viện' }]} className="w-full">
                <Select placeholder="Chọn khoa">
                  {listDepartment.map(department => (
                    <Select.Option key={department.id} value={department.id}>
                      {department.name}
              ...
              <Form.Item name="DiagnosisTransfer" label="Chẩn đoán" rules={[{ required: true, message: 'Vui lòng nhập chẩn đoán chuyển viện' }]} className="w-full">
                <Input />
              </Form.Item>
            </Panel>
          </Collapse>
...
export default MedicalManagementByNurse
