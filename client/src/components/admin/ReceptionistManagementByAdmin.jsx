...
import { createStaff, getReceptionist, deleteStaff, updateStaff} from "../../Api";
...
const ReceptionistManagementByAdmin = () => {
        ...
        },
        {
          ...
          render: (_, receptionist) => (
            <Space size="middle">
              ...
              <Button type="link" className="delete" onClick={() => handleDeleteReceptionist(receptionist.Id)}>
                Xóa
              </Button>
            </Space>
          ),
    ...
    const fetchReceptionist = async () => {
        try {
          let response = await axios.get(getReceptionist("", "", "", ""), {
    ...
    const handleSearch = async () => {
        try {
          let response = await axios.get(getReceptionist(keyword), {
            withCredentials: true
          });
          if (response.status === 200) {
